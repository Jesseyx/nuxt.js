...
module.exports = class Renderer {
  ...
  async renderRoute(url, context = {}) {
    ...
    const BUNDLE_APP = APP
    const BUNDLE_SESSION = `<script type="text/javascript">${serializedSession}</script>`
    const SSR_SCRIPTS = context.renderScripts()
    let BUNDLE_SCRIPTS = SSR_SCRIPTS

    APP += BUNDLE_SESSION
    APP += SSR_SCRIPTS
    const bodyScript = m.script.text({ body: true })
    APP += bodyScript
    const bodyNoScript = m.noscript.text({ body: true })
    APP += bodyNoScript

    BUNDLE_SCRIPTS += bodyScript
    BUNDLE_SCRIPTS += bodyNoScript
